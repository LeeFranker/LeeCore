...
public class HttpAndroidUtils {
	...
	/**
	 * 生成Http消息头中的ContentType的值。 Content-Type为multipart/form-data
	 * 
	 * @param boundary
	 * @param charset
	 * @return
	...
	/**
	 * 生成Http消息头中的ContentType的值 Content-Type为application/x-www-form-urlencoded
	 * 
	 * @param charset
	 * @return
	 */
	...
	/**
	 * 将Cookie中的domain和path值和请求的url进行筛选匹配。 匹配规则：
	 * 1.请求的主机名是否与某个存储的Cookie的Domain属性尾部匹配
	 * ；如acme.com的domain将与主机名anvil.acme.com匹配； 2.请求的端口号是否在该Cookie的Port属性列表中；
	 * 3.请求的资源路径是否在该Cookie的Path属性指定的目录及子目录中，即是否与Path属性头部匹配； 4.该Cookie的有效期是否已过。
	 * 结果： 如果匹配，则返回true(验证通过，会把该Cookie加入这个请求中)； 如果不匹配，返回false。 注意：url的格式规范为：
	...
	/**
	 * 输入Content-Type的值，获取响应头中的字符编码。例： Content-Type:text/html;charset=ISO-8859-1
	 * 
